...
int ZlibStream::init(Type type, int level, int window_bits
    ...
    LJY_ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    LJY_ASSERT((window_bits >= 8 && window_bits <= 15));
    LJY_ASSERT((memlevel >= 1 && memlevel <= 9));
...
ljy::ByteArray::ptr ZlibStream::getByteArray() {
    ljy::ByteArray::ptr ba(new ljy::ByteArray);
    for(auto& i : m_buffs) {
