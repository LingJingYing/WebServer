...
void Scheduler::startLoop(){
    ...
    m_threadPoll.resize(m_threadCount-1);
...
void Scheduler::run(){
    LJY_LOG_DEBUG(g_logger) << "Scheduler:" << m_name << " run";
...
*/
    ...
    ljy::Fiber::GetCurrentFiber();//创建主协程

    ljy::Fiber::ptr idleFiber(new Fiber(std::bind(&ljy::Scheduler::idle, this)));//idle协程
    Fiber::ptr cb_fiber;
...
bool Scheduler::stopping(){
   MutexType::Lock lock(m_mutex);
    return m_started && m_stopping //已经启动协程，已经调用Scheduler::stop
