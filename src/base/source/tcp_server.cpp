...
static ljy::ConfigVar<uint64_t>::ptr g_tcp_server_read_timeout =
    ljy::Config::Lookup("tcp_server.read_timeout", (uint64_t)(60 * 1000 * 2),
            "tcp server read timeout");

static ljy::Logger::ptr g_logger = LJY_LOG_NAME("system");

TcpServer::TcpServer(ljy::IOManager* worker,
...
bool TcpServer::bind(ljy::Address::ptr addr, bool ssl) {
...
bool TcpServer::bind(const std::vector<Address::ptr>& addrs
    ...
    for(auto& addr : addrs) {
        ...
        if(!sock->bind(addr)) {
            LJY_LOG_ERROR(g_logger) << "bind fail errno="
        ...
        if(!sock->listen()) {
            LJY_LOG_ERROR(g_logger) << "listen fail errno="
                << errno << " errstr=" << strerror(errno)
                << " addr=[" << addr->toString() << "]";
            fails.push_back(addr);
            continue;
    ...
    if(!fails.empty()) {
        m_socks.clear();//析构时执行close
        return false;
    }

    for(auto& i : m_socks) {
        LJY_LOG_INFO(g_logger) << "type=" << m_type
            << " name=" << m_name
            << " ssl=" << m_ssl
...
void TcpServer::startAccept(Socket::ptr sock) {
    while(!m_isStop) {
        ...
        if(client) {
            ...
            LJY_LOG_ERROR(g_logger) << "accept errno=" << errno
...
void TcpServer::handleClient(Socket::ptr client) {
    LJY_LOG_INFO(g_logger) << "handleClient: " << *client;
}
