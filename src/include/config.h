...
template<class T, class FromStr = LexicalCast<std::string, T>
                ,class ToStr = LexicalCast<T, std::string> >//模板类 默认值
class ConfigVar : public ConfigVarBase {//观察者模式
public:
...
private:
    ...
    std::map<uint64_t, on_change_cb> m_cbs;//观察者模式
};
...
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,
        ...
        if(name.find_first_not_of("abcdefghikjlmnopqrstuvwxyz._012345678")//find_first_not_of：正向查找在原字符串中第一个与指定字符串（或字符）中的任一字符都不匹配的字符，返回它的位置。若查找失败，则返回npos。
                != std::string::npos) {
     ...
     * @param[in] cb 配置项回调函数
     */
    static void Visit(std::function<void(ConfigVarBase::ptr)> cb);//刷新一遍配置
private:
