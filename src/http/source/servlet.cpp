...
int32_t FunctionServlet::handle(ljy::http::HttpRequest::ptr request
...
ServletDispatch::ServletDispatch()
    ...
    m_default.reset(new NotFoundServlet("ljy/1.0"));
}

int32_t ServletDispatch::handle(ljy::http::HttpRequest::ptr request
               , ljy::http::HttpResponse::ptr response
               , ljy::http::HttpSession::ptr session) {
...
void ServletDispatch::addServlet(const std::string& uri, Servlet::ptr slt) {
    ...
    //返回指向此对象的shared_ptr;由于是通过shared_ptr管理内存，因此一种安全分配和使用动态内存的方法。
...
void ServletDispatch::addGlobServlet(const std::string& uri
    ...
    m_globs.push_back(std::make_pair(uri//再添加
...
int32_t NotFoundServlet::handle(ljy::http::HttpRequest::ptr request
                   , ljy::http::HttpResponse::ptr response
                   , ljy::http::HttpSession::ptr session) {
    response->setStatus(ljy::http::HttpStatus::NOT_FOUND);
    response->setHeader("Server", "ljy/1.0.0");
